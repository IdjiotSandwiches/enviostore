                ...
                <tbody>
                    @foreach ($products as $product)
                        <tr class="bg-white border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-4">
                                <form action="{{ route('admin.product.deleteProduct', $product->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this product?')">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <td class="py-4">
                                <a href="{{ route('admin.product.editProduct', $product->id) }}" class="bg-blue-600 text-white font-medium py-2 px-4 rounded-lg hover:bg-blue-700">{{ __('page.admin.edit') }}</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
