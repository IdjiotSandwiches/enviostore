...
<section class="flex items-center justify-center px-4 py-8">
    ...
    <div class="w-full max-w-md p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8">
        <form action="{{ route('attemptRegister') }}" method="POST" class="space-y-4">
            @csrf
            <h5 class="text-3xl text-center font-bold text-gray-900">{{ __('register.register') }}</h5>
            <div>
                <label for="username" class="block mb-2 text-sm font-medium text-gray-900">{{ __('register.username') }}</label>
            ...
            <div>
                <label for="phone_number" class="block mb-2 text-sm font-medium text-gray-900">{{ __('register.phone') }}</label>
            ...
            <div>
                <label for="password_confirmation" class="block mb-2 text-sm font-medium text-gray-900">{{ __('register.confirmation') }}</label>
            ...
            <div>
                <div class="flex items-center">
                    ...
                    <label for="checkbox" class="ms-2 text-sm font-medium text-font_primary">
                        {{ __('register.agree') }} <a href="#" class="text-font_secondary lowercase hover:text-font_primary hover:underline">{{ __('footer.legal.terms') }}</a>
                    </label>
                </div>
            ...
            <div class="text-sm font-medium text-font_primary">
                {{ __('register.have') }}? <a href="{{ route('login') }}" class="text-font_secondary hover:text-font_primary hover:underline">{{ __('login.login') }}</a>
            </div>
        </form>
    </div>
</section>
