...
<section class="flex items-center justify-center px-4 py-8">
    ...
    <div class="w-full max-w-md p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8">
        <form action="{{ route('attemptLogin') }}" method="POST" class="space-y-4">
            @csrf
            <h5 class="text-3xl text-center font-bold text-gray-900">{{ __('title.login') }}</h5>
            <div>
            ...
            <div class="flex items-start">
                <a href="#" class="text-sm text-font_secondary hover:text-font_primary hover:underline">{{ __(key: 'form.lost') }}</a>
            </div>
            ...
            <div class="text-sm font-medium text-font_primary">
                {{ __('form.not') }}? <a href="{{ route('register') }}" class="text-font_secondary hover:text-font_primary hover:underline">{{ __('form.create') }}</a>
            </div>
        </form>
    </div>
</section>
