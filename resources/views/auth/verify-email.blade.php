...
<section class="max-w-screen-xl p-4 md:px-4 md:py-8 md:mx-auto flex justify-center gap-4">
    <div class="max-w-lg p-4 bg-white shadow rounded-lg sm:p-8 text-center flex flex-col items-center justify-center gap-4">
        ...
        <h2 class="text-2xl font-bold text-font_primary">Verify Your Email Address</h2>
        ...
        <form method="POST" action="{{ route('verification.send') }}">
            @csrf
            <button type="submit" class="w-full text-white bg-button hover:bg-button/80 focus:ring-4 focus:outline-none focus:ring-button/15 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-button dark:hover:bg-button/80 dark:focus:ring-button/15">Resend Verification Email</button>
        </form>
        @if (session('message'))
            <div>
                {{ session('message') }}
